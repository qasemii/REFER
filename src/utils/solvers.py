...
def top_k_perecent(attrs: Tensor, k: int, min_val: float = -1e10) -> Tensor:
    ...
    num_highlight_tokens = torch.maximum(num_highlight_tokens, ones).long().item()
    ...
    scores, indices = torch.topk(attrs, num_highlight_tokens, sorted=True)
    thr_2d = scores[:, -1].view(-1, 1)
    expl = (attrs >= thr_2d).float()
